  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-text-muted">Koszty materiałów i usług z pełnymi danymi księgowymi.</p>
      ...
      {formOpen ? (
        <div className="border border-border rounded-xl p-4 bg-bg-secondary space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <label className="block space-y-1">
              <span className="text-sm text-text">Typ dokumentu</span>
            ...
            {form.amountSource === "gross" ? (
              <label className="block space-y-1">
                <span className="text-sm text-text">Brutto (PLN) *</span>
              ...
              <label className="block space-y-1">
                <span className="text-sm text-text">Netto (PLN) *</span>
            ...
            <label className="block space-y-1">
              <span className="text-sm text-text">Stawka VAT</span>
            ...
            {form.vatSelect === BUDGET_VAT_CUSTOM ? (
              <label className="block space-y-1">
                <span className="text-sm text-text">Własna stawka (%)</span>
      ...
      {loading && items.length === 0 ? (
        <div className="text-sm text-text-muted py-8 text-center">Ładowanie kosztów...</div>
      ) : items.length === 0 ? (
        <div className="text-sm text-text-muted py-8 text-center border border-border rounded-xl bg-bg-secondary">Brak pozycji kosztowych.</div>
      ) : (
            ...
            <tbody className="divide-y divide-border">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-card-hover">
                  ...
                  <td className="py-3 pr-3 text-text-muted">{item.supplier ?? "—"}</td>
                  <td className="py-3 pr-3 text-text-muted">{item.category}</td>
